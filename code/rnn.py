...
class RNN(object):
    ...
    def acc_deltas_np(self, x, d, y, s):
        ...
        del_W += np.outer(der_softmax, s[len(x) - 1])
        del_V += np.outer(del_inputs, make_onehot(x[len(x) - 1], self.vocab_size))
        del_U += np.outer(del_inputs, s[len(x) - 1 - 1])
    ...
    def compute_loss_np(self, x, d):
        ...
        d_1h = make_onehot(int(d), self.out_vocab_size)
        loss = -np.sum(d_1h * np.log(pred))
        return loss
    ...
    def compare_num_pred(self, x, d):
        '''
                        ...
                        '''
        y_pred, hiddens = self.predict(x)
        print(y_pred[(len(y_pred) - 1)][d[0]])

        if y_pred[(len(y_pred) - 1)][d[0]] > y_pred[(len(y_pred) - 1)][d[1]]:
            return 1
        else:
            return 0
    ...
    def train_np(self, X, D, X_dev, D_dev, epochs=10, learning_rate=0.5, anneal=5, back_steps=0, batch_size=100,
        ...
        np.save('rnn-np.U.npy', self.U)
        np.save('rnn-np.V.npy', self.V)
        np.save('rnn-np.W.npy', self.W)
...
if __name__ == "__main__":
    ...
    if mode == "test-lm":
        ...
        t = RNN(vocab_size, hdim, vocab_size)
        t.U = U
        t.V = V
        t.W = W
        ...
        mean_loss = t.compute_mean_loss(X_test, D_test)
        
        print("test_length =", "X", len(X_test), "D", len(D_test))  
        ...
        fracloss = fraq_loss(vocab, word_to_num, vocab_size)
        q = vocab.freq[vocab_size] / sum(vocab.freq[vocab_size:])         
        adjusted_loss = adjust_loss(mean_loss,fracloss, q)
